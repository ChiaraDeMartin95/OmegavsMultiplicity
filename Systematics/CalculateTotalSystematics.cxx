...
void CalculateTotalSystematics() {
  ...
  // Anchoring
  TH1D *hSystAnch = (TH1D *)hPt->Clone("hSystAnch");
  for (int i = 1; i <= hSystAnch->GetNbinsX(); i++)
  {
    hSystAnch->SetBinContent(i, 0.04);
  }
  prephisto(hSystAnch, kYellow+2);

  // Anchoring
  TH1D *hSystSgnLoss = (TH1D *)hPt->Clone("hSystSgnLoss");
  for (int i = 1; i <= hSystSgnLoss->GetNbinsX(); i++)
  {
    hSystSgnLoss->SetBinContent(i, 0.015);
  }
  prephisto(hSystSgnLoss, kViolet);
