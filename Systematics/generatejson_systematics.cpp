// The macro generates 100 different jsons with 100 different cut settings
                         ...
                         float isSelectBachBaryon, int isXi, int evSelFlag, int processGen, int processRec)
{
    ...
    json << "        \"casccospa\": \"" << std::fixed << std::setprecision(3) << casccospa << "\"," << std::endl;
...
int generatejson_systematics()
{
    ...
    float bachBaryonDCAxyToPV = 0.015;
    float casccospa = 0.99;
    ...
    float minpt = 0;
    int mintpccrrows = 50;
    float nsigmatofKa = 10;
    float nsigmatofPi = 10;
    float nsigmatofPr = 10;
    float nsigmatpcKa = 3.5;
    float nsigmatpcPi = 4;
    float nsigmatpcPr = 4;
    float processGen = 0;
    float processRec = 1;
    ...
    std::string jsondef = generateJson(bachBaryonCosPA, bachBaryonDCAxyToPV, casccospa, cascradius, dcabachtopv,
                                       ...
                                       v0radius, hastof, isMC, isSelectBachBaryon, isXi, evSelFlag, processGen,
                                       processRec);
    ...
    std::string jsondefMC = generateJson(bachBaryonCosPA, bachBaryonDCAxyToPV, casccospa, cascradius, dcabachtopv,
                                         ...
                                         nsigmatofKa, nsigmatofPi, nsigmatofPr, proplifetime, ptthrtof, rejcomp, v0cospa,
                                         v0radius, hastof, ifmc, isSelectBachBaryon, isXi, evSelFlag, ifmc,
                                         processRec);
    ...
    //Limits:
    double casccospalim[2] = {0.97, 0.995};
    ...
    double dcav0topvlim[2] = {0.03, 0.1};
    ...
    double rejcomplim[2] = {0.008, 0.016};

    const int njson = 500;

    std::string filename[njson+1];
    std::string filenameMC[njson + 1];
    ...
    for (int i = 0; i <= njson; i++)
    {
        ...
        filename[i] = "DATA/config/systjson" + std::to_string(i) + ".json";
        filenameMC[i] = "MC/config/systjson" + std::to_string(i) + ".json";
